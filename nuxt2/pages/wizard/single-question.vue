<script setup lang="ts">
  ...
  import ConfirmationStep from '../../components/wizard/steps/ConfirmationStep.vue'
  import AnalyzingStep from '../../components/wizard/steps/AnalyzingStep.vue'
  import ConceptEditor from '../../components/wizard/steps/ConceptEditor.vue'
  import FrequencyTable from '../../components/wizard/steps/FrequencyTable.vue'
  ...
  const handleAnalysisComplete = () => {
    ...
    const mockMapping: ConceptMapping[] = answers.value.map((answer, index) => {
      ...
      const mockItem = MOCK_ANALYSIS_RESULT.mapping.find(
        (m) => m.answer_index === index + 1
      )
      return {
        ...
        concepts:
          mockItem?.concepts ||
          mockConcepts.slice(0, Math.floor(Math.random() * 3) + 1),
      }
    })
...
<template>
  <div class="min-h-screen bg-background flex flex-col">
    <!-- Header -->
    <header
      class="h-14 border-b border-border bg-card/50 backdrop-blur-sm flex items-center px-4 sticky top-0 z-50"
    >
      <Button
        variant="ghost"
        size="sm"
