<script setup lang="ts">
  ...
  const handleDownload = () => {
    ...
    const rows = frequencyData.value.map((d) => [
      d.concept,
      d.count.toString(),
      `${d.percentage}%`,
    ])
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="p-6 border-b border-border bg-muted/30">
      <div class="flex items-start justify-between">
        ...
        <div class="flex items-center gap-2">
          <Button variant="outline" class="gap-2" @click="handleDownload">
      ...
      <div class="w-1/2 p-6 border-r border-border">
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <div
              class="flex-1 h-8 bg-muted rounded-md overflow-hidden relative"
            >
              <div
          ...
          <table class="w-full">
            <thead class="sticky top-0 bg-muted/50">
              <tr class="border-b border-border">
                <th
                  class="text-left p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide"
                >
                  Concepto
                </th>
                <th
                  class="text-center p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide w-24"
                >
                  Frec.
                </th>
                <th
    ...
    >
      <Button variant="outline" class="gap-2" @click="onBack">
        <ArrowLeft class="w-4 h-4" />
        Volver a editar
      </Button>
      <Button class="gap-2" @click="onGoHome">
        <Home class="w-4 h-4" />
        Ir al inicio
      </Button>
    </div>
