<script setup lang="ts">
  ...
  const addNewConcept = () => {
    if (
      !newConceptName.value.trim() ||
      concepts.value.includes(newConceptName.value.trim())
    ) {
      return
    }
  ...
  const handleDrop = (e: DragEvent, answerIndex: number) => {
    ...
    mapping.value = mapping.value.map((m) => {
      if (
        m.answer_index === answerIndex &&
        !m.concepts.includes(draggedConcept.value as string)
      ) {
...
<template>
  <div class="flex h-full">
    ...
    <div class="w-80 border-r border-border flex flex-col bg-muted/20">
      ...
      <ScrollArea class="flex-1">
        <div class="p-2 space-y-1">
          ...
          >
            ...
            <div
              v-if="editingConcept?.index === index"
            ...
            >
              <Input
                ...
                autofocus
                @keydown.enter="
                  updateConceptGlobally(concept, editingConcept.value)
                "
      ...
      <div
        class="p-3 border-t border-border text-xs text-muted-foreground text-center"
      >
    ...
    <div class="flex-1 flex flex-col">
      <div
        class="p-4 border-b border-border bg-muted/30 flex items-center justify-between"
      >
        <div>
          <h3 class="font-semibold text-sm">
            Respuestas y conceptos asignados
          </h3>
          <p class="text-xs text-muted-foreground mt-0.5">
            Arrastra conceptos de la izquierda o haz clic en + para asignarlos
          </p>
        </div>
        <Button class="gap-2" @click="handleSave">
          <Save class="w-4 h-4" />
          Guardar y continuar
        </Button>
      </div>
        ...
        <div class="p-4 space-y-3">
          ...
          >
            <div class="flex items-start gap-3 mb-3">
              <span
                class="text-xs font-mono text-muted-foreground bg-muted px-2 py-1 rounded flex-shrink-0"
              >
                #{{ item.answer_index }}
              </span>
            ...
            <div class="flex flex-wrap gap-2 min-h-[32px]">
              ...
              <div class="relative group">
                <button
                  class="h-6 px-2 rounded-md border border-dashed border-border text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors flex items-center gap-1"
                >
                  <Plus class="w-3 h-3" />
                  Agregar
                </button>
                ...
                <!-- Quick add dropdown -->
                <div
                  class="absolute left-0 top-full mt-1 z-20 hidden group-hover:block"
                >
                  <div
                    class="bg-popover border border-border rounded-lg shadow-lg p-2 max-h-48 overflow-auto w-48"
                  >
                    <button
                    ...
                    <span
                      v-if="
                        concepts.filter((c) => !item.concepts.includes(c))
                          .length === 0
                      "
