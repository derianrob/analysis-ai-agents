<script setup lang="ts">
  ...
  const cells = ref<string[]>([EXAMPLE_DATA.question, ...EXAMPLE_DATA.answers])
  ...
  const handleConfirm = (
    confirmFn: (question: string, answers: string[]) => void
  ) => {
  ...
  <div class="flex flex-col h-full">
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent class="max-w-xs">
              <p>
                Pega o edita tus datos. Usa los botones de la izquierda para
                marcar cuál es la pregunta (P) y cuáles son las respuestas (R).
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-2 text-sm text-muted-foreground">
          <span class="flex items-center gap-1">
            <span
              class="w-6 h-6 rounded bg-primary text-primary-foreground flex items-center justify-center text-xs font-medium"
            >
              P</span
            >
            Pregunta
          ...
          <span class="flex items-center gap-1">
            <span
              class="w-6 h-6 rounded bg-emerald-500 text-white flex items-center justify-center text-xs font-medium"
            >
              R</span
            >
            Respuestas
    ...
    <div class="flex-1 overflow-auto">
      <div class="min-w-[600px]">
        <!-- Column headers -->
        <div
          class="flex items-center border-b border-border bg-muted/50 sticky top-0 z-10"
        >
          <div
            class="w-12 p-2 text-center text-xs font-medium text-muted-foreground border-r border-border"
          >
            #
          </div>
          <div
            class="w-14 p-2 text-center text-xs font-medium text-muted-foreground border-r border-border"
          >
            Tipo
          </div>
          <div class="flex-1 p-2 text-xs font-medium text-muted-foreground">
            Contenido
          </div>
          <div
            class="w-12 p-2 text-center text-xs font-medium text-muted-foreground"
          />
        </div>
        ...
        >
          <!-- Row number -->
          <div
            class="w-12 p-2 text-center text-xs text-muted-foreground border-r border-border flex items-center justify-center"
          >
            {{ index + 1 }}
          </div>
          ...
          <!-- Type toggles -->
          <div
            class="w-14 p-1 border-r border-border flex items-center justify-center gap-1"
          >
            <button
              :class="
                cn(
          ...
          <div class="flex-1 p-0">
            <textarea
              :ref="
                (el) => (inputRefs[index] = el as HTMLTextAreaElement | null)
              "
              :value="cell"
              :class="
                cn(
    ...
    >
      <div class="text-sm text-muted-foreground">
        <span v-if="questionRow !== null" class="text-foreground">
          1 pregunta seleccionada</span
        >
        <span v-else class="text-amber-500">Selecciona una pregunta (P)</span>
        <span class="mx-2">•</span>
        <span v-if="answerRows.size > 0" class="text-foreground">
          {{ answerRows.size }} respuestas seleccionadas</span
        >
